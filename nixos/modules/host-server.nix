...
# SPDX-License-Identifier: Apache-2.0
{
  ...
  cfg = config.givc.host-server;
  givc-host-server = pkgs.callPackage ../packages/givc-host-server.nix {};
  inherit (lib) mkOption mkEnableOption mkIf types trivial concatStringsSep attrsets;
in {
  options.givc.host-server = {
    enable = mkEnableOption "Enable givc-host-server module.";
    ...
    port = mkOption {
      description = "Port of the host-server service. Defaults to '9000'.";
      type = types.str;
      default = "9000";
    };
  ...
  config = mkIf cfg.enable {
    ...
    systemd.services.givc-host-server = {
      ...
      serviceConfig = {
        Type = "exec";
        ExecStart = "${givc-host-server}/bin/givc-host-server";
        Restart = "always";
        RestartSec = 1;
      };
      environment =
