...
func main() {
	...
	// Create admin server
	hostServer := systemmanager.NewHostServer(&cfgServer)

	// Start monitoring
	go func() {
		time.Sleep(4 * time.Second)
		hostServer.HostService.Monitor()
	}()

	// Start server
	grpcServer, err := givc_grpc.NewServer(&cfgServer, []types.GrpcServiceRegistration{hostServer})
	if err != nil {
